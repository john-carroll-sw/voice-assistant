class SpeechInputButton extends HTMLElement {
  constructor() {
    ...
    if (!SpeechRecognition) {
      alert("Speech recognition is not supported in this browser. For best results, use Google Chrome. (Edge on macOS may not support the Web Speech API.)");
      this.innerHTML = `<button class="btn btn-outline-secondary" type="button" disabled title="Speech recognition not supported">
        <i class="bi bi-mic"></i>
      </button>`;
      return;
    }
    this.speechRecognition = new SpeechRecognition();
    // Force language to en-US for maximum compatibility on Chrome and Edge
    this.speechRecognition.lang = "en-US";
    this.speechRecognition.interimResults = false;
    this.speechRecognition.continuous = true;
    this.speechRecognition.maxAlternatives = 1;

    // Show a warning only for Edge browsers on macOS
    const isEdge = navigator.userAgent.includes("Edg");
    const isMac = navigator.platform.toLowerCase().includes("mac");
    if (isEdge && isMac) {
      setTimeout(() => {
        alert("Speech recognition is only partially supported in Microsoft Edge on macOS. For best results, use Google Chrome.");
      }, 500);
    }
  }
